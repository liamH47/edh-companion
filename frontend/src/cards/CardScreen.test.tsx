...
beforeEach(() => {
  vi.mocked(api.calculateCard).mockResolvedValue({ outputs: { damage_available: 50, current_life: 90 } })
})
...
describe('CardScreen', () => {
  ...
  it('shows an error banner when calculation fails', async () => {
    vi.mocked(api.calculateCard).mockRejectedValue(new Error('network down'))
    render(<CardScreen card={aetherfluxLikeCard} />)
  ...
  it('renders an all-setup card inline with no summary bar or sheet', async () => {
    vi.mocked(api.calculateCard).mockResolvedValue({ outputs: { total: 0 } })
  ...
  it('renders only the hero for a single-output card, no stat strip', async () => {
    vi.mocked(api.calculateCard).mockResolvedValue({ outputs: { total_life_drained: 6 } })

 ...
 *
 * This package runs on node by default -- almost all of it is pure. React hooks
 * need somewhere to render, so they opt in here rather than making every other
 * file pay for a DOM it never touches.
 */
import { resetStorageBackend } from './storage'
import { act, renderHook } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { setComputeBackend } from './compute'
...
/** Both registered as backends in beforeEach, below. */
const mockedCalculateCard = vi.fn()
...
beforeEach(() => {
  vi.useFakeTimers()
  resetStorageBackend()
  mockedCalculateCard.mockReset()
  setComputeBackend(mockedCalculateCard)
